...
void solve()
{
    ...
    int mininfect = n, maxinfect = 0, prev = 0;
    for (int i = 1; i < n; i++)
    {
        if (person[i] > person[i-1] + 2)
        {
            mininfect = min(mininfect, i-prev);
            maxinfect = max(maxinfect, i-prev);
            prev = i;
        }
    }
    if(person[n-1] <= person[n-1]+2){
        mininfect = min(mininfect, n-prev);
        maxinfect = max(maxinfect, n-prev);
    }
